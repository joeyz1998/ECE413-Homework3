...
switch choice
    ...
    case 18
        file = fopen('midi/surprise.mid'); % insert any file here and press button
    otherwise
end
mid = fread(file);
fclose(file);
                ...
                switch bin(2:4) % next three bits, command code
                    case {'001'} % note on
                        ...
                        if l2 ~= 0
                            ...
                            if chan == 9 % drum on chan 9, means inst = 2
                                song(sonl,:) = [l1, temp, key, time(track_num) + del, 0, amp*(l2/127), chan, 2];
                            else % instrument is hard coded at 1 because I like it
                                song(sonl,:) = [l1, temp, key, time(track_num) + del, 0, amp*(l2/127), chan, 1];
                            end
                            sonl = sonl + 1;
