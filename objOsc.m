                ...
                if all(mask == 0)
                    ...
                    switch obj.note.instrument
                        ...
                        case 2
                            ...
                            %audio = zeros(1,obj.constants.BufferSize).';
                        case 3 % not currently being used
                            fmod1 = .4 .* sin( 2 * pi * 10 * timeVec);
                            fmod = 2 * mask(:) .* sin( 2 * pi * (5/5) * freq * timeVec + fmod1);
                            audio = obj.note.amplitude * mask(:) .* sin( (2 * pi * freq * timeVec) + fmod);
                        case 4 % not currently being used
                            fmod1 = .2 .* sin( 2 * pi * 10 * timeVec);
                            fmod = 6 * mask(:) .* sin( (2 * pi * (4/7) * freq * timeVec) + fmod1);
                            audio = obj.note.amplitude * mask(:) .* sin( (2 * pi * freq * timeVec) + fmod);
                        otherwise
                            audio = zeros(1,obj.constants.BufferSize).';
